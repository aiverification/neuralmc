...
def spec_automata(ctx, q_cur, curr_vars, V0, q_nex, next_vars, V1, non_state, s):
    ...
    if q_cur == 0 and q_nex == 1:
        ...
        cases.append([nuR.BUnSet(non_state, 'lb_pulse', 1, ctx), nuR.BUnSet(non_state, 'pulse_red', 1, ctx)])
        cases.append([nuR.Bset(  non_state, 'ub_pulse', 1, ctx), nuR.Bset(  non_state, 'pulse_red', 1, ctx)])
    elif q_cur == 1 and q_nex == 2:
        cases.append([nuR.Bset(  non_state, 'lb_pulse', 1, ctx), nuR.BUnSet(non_state, 'pulse_red', 1, ctx)])
        cases.append([nuR.BUnSet(non_state, 'ub_pulse', 1, ctx), nuR.Bset(  non_state, 'pulse_red', 1, ctx)])
