...
def spec_automata(ctx, q_cur, curr_vars, V0, q_nex, next_vars, V1, non_state, s):
    ...
    if q_cur == 0 and q_nex == 0:
        ...
        cases.append([nuR.Bset(non_state, 'rst', 0, ctx),  nuR.BUnSet(non_state, 'sig', 1, ctx),  nuR.BUnSet(non_state, 'flg', 1, ctx)])
    elif q_cur == 0 and q_nex == 3:
        cases.append([nuR.Bset(non_state, 'rst', 0, ctx),  nuR.BUnSet(non_state, 'sig', 1, ctx),  nuR.Bset(non_state, 'flg', 1, ctx)])
    
    elif q_cur == 2 and q_nex == 1:
        cases.append([nuR.Bset(non_state, 'rst', 0, ctx),  nuR.Bset(non_state, 'sig', 1, ctx)])
        cases.append([nuR.Bset(non_state, 'rst', 0, ctx),  nuR.Bset(non_state, 'flg', 1, ctx)])
