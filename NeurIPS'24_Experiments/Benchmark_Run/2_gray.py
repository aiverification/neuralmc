...
lrs = [0.5]*len(CBITSs)#[0.5]*5 + [0.05] + [0.1] + [0.05] + [0.1] + [0.5]*10
q_skews = [50]*len(CBITSs)
rst_skews = [50]*len(CBITSs)
sizes = [(50000, 10)]*5 + [(100000, 10)]*5 + [(200000, 10)]*10 
caps = [10]*len(CBITSs)
...
for dut_i in range(len(CBITSs)):
	...
	name = f"gray_{dut_i+1}"
	smt_file_name = f"2_{name}_{network_type}_{SMTencode}"
	module_name = "GRAY"
	F_prec = 14
	idtxt = f"{name} ({specTXT}) {CBITS}"
	...
	if ch == 1:
		nuXmvSpec = "LTLSPEC F G (Verilog.GRAY.rst = FALSE) -> G F (Verilog.GRAY.sig = TRUE & X Verilog.GRAY.sig = FALSE)"
		abc_run.runABC(name, module_name, nuXmvSpec, idtxt)
		continue
	
	if ch == 2:
		nuXmvSpec = "LTLSPEC F G (Verilog.GRAY.rst = FALSE) -> G F (Verilog.GRAY.sig = TRUE & X Verilog.GRAY.sig = FALSE)"
		nuxmv_run.runNuXmv(name, module_name, nuXmvSpec, idtxt)
		continue
		   ...
		   "if (q == 0 && (obj.sig != 1 && rst == 0)) begin",
		   	f"\tif ($urandom % {q_skew} == 0)",
		   	 ...
		   	 "\telse",
		   	 	"\t\tq = 0;",
		   	 "end",
		   "else if (q == 0 && (obj.sig == 1 && rst == 0)) begin",
		   	f"\tif ($urandom % {q_skew} == 0)",
		   	 	"\t\tq = 2;",
		   	 "\telse",
		   	 	"\t\tq = 0;",
	...
	spec = [ [nuR.Bset(curr_vars, 'q', 0, ctx), nuR.Bset(next_vars, 'q',0, ctx)],
			 ...
			 [nuR.Bset(curr_vars, 'q', 0, ctx), nuR.Bset(non_state_vars, 'sig', 1, ctx), nuR.Bset(non_state_vars, 'rst', 0, ctx), nuR.Bset(next_vars, 'q', 2, ctx)],
			 ...
			 [nuR.Bset(curr_vars, 'q', 1, ctx), nuR.Bset(non_state_vars, 'sig', 1, ctx), nuR.Bset(non_state_vars, 'rst', 0, ctx), nuR.Bset(next_vars, 'q', 2, ctx)],
			 [nuR.Bset(curr_vars, 'q', 2, ctx), nuR.Bset(non_state_vars, 'sig', 1, ctx), nuR.Bset(non_state_vars, 'rst', 0, ctx), nuR.Bset(next_vars, 'q', 2, ctx)]
			 ]	
