...
for dut_i in range(len(N_lims)):
	...
	spec_str = ["isValid = 1;",
			   ...
			   "else if (q == 0)",
			   	"\tq = 0;",
			   "else if (q == 1 && (obj.digit_select != 1 && rst == 0))",
			   	"\tq = 1;", 
			   "else",
			   	"\tisValid = 0;"]
	stt_acc = [1]
	...
	spec = [ [nuR.Bset(curr_vars, 'q', 0, ctx), nuR.Bset(next_vars, 'q',0, ctx)],
			 [nuR.Bset(curr_vars, 'q', 0, ctx), nuR.BUnSet(next_vars, 'digit_select', 1, ctx), nuR.Bset(non_state_vars, 'rst', 0, ctx), nuR.Bset(next_vars, 'q', 1, ctx)],
			 [nuR.Bset(curr_vars, 'q', 1, ctx), nuR.BUnSet(next_vars, 'digit_select', 1, ctx), nuR.Bset(non_state_vars, 'rst', 0, ctx), nuR.Bset(next_vars, 'q', 1, ctx)]]	
